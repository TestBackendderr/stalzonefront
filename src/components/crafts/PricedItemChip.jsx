...
function PricedItemChip({ item, locale, size = 'md' }) {
  ...
  const unitPrice = item.auction?.minBuyoutPerUnit
  const lineTotal = item.auction?.lineTotal
  ...
  return (
    <div className="flex items-center justify-between gap-3 min-w-0">
      ...
      <div className="shrink-0 text-right text-xs">
        <p className="text-zone-muted">
          {formatPrice(unitPrice)}
          <span className="text-[10px]"> / шт</span>
        </p>
        <p className="font-medium text-zone-text">{formatPrice(lineTotal)}</p>
      </div>
    </div>
  )
}

export default PricedItemChip
